...
export default function WorldIntroSceneV2({
  ...
  // إذا كان المشهد يحتوي text نعرضه بدل نص JSON
  // هذا يسمح بتوحيد بدايات الدروس بصريًا قبل إعادة توليد الصوت والكرايوكي.
  const overrideText = (slide.text || "").trim();
  const displayWords: WordTiming[] | null = overrideText
    ? overrideText.split(/\s+/).map((w) => ({ text: w, offset: 0, duration: 0 }))
    : words;

  const hasOverrideText = !!overrideText;
        ...
        <div onClick={replayCurrent} style={{
          ...
          {displayWords ? (
            displayWords.map((w, i) => {
              const sceneActiveTxt = !hasOverrideText && karaoke.activeKey === slide.audio_key;
              const isShown = hasOverrideText ? true : (sceneActiveTxt ? karaoke.shown.has(i) : true);
              const isCurrent = !hasOverrideText && sceneActiveTxt && karaoke.currentIdx === i;
              return (
                <span key={i} style={{
                  display: "inline-block",
