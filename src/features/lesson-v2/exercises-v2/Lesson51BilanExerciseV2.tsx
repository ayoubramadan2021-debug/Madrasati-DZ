...
export type Lesson51BilanExerciseV2Props = {
  items: readonly Lesson51Question[];
  audio_base: string;
  missionTitle: string;
  onComplete?: (
    score: number,
...
export default function Lesson51BilanExerciseV2({
  items,
  audio_base,
  missionTitle,
  onComplete,
}: Lesson51BilanExerciseV2Props) {
  ...
  const displayFocusEmoji =
    selectedOption &&
    /[?❓]/.test(question.focusEmoji)
      ? question.focusEmoji.replace(
          /[?❓]/,
          selectedOption.content,
        )
      : question.focusEmoji;

  const focusTokenCount =
    question.focusEmoji
      .trim()
      .split(/\s+/)
      .filter(Boolean)
