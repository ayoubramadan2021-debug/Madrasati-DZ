...
function DraggableChip({
  ...
  return (
    ...
    >
      {isMatched ? (
        <span style={{ fontSize: 32, fontWeight: 900, color: "#888" }}>✓</span>
      ) : (
...
export default function DragMatchExerciseV2({
  ...
  const handleDragEnd = (e: DragEndEvent) => {
    ...
    if (draggedMatchId === targetMatchId) {
      ...
      setTimeout(() => setWrongMatchId(null), 1200);
    }
  };
  ...
  const progressEmoji = "🍊";
  const missionText = `مهمة الربط ${itemIdx + 1}`;

  const coachText =
    feedbackState === "complete"
      ? "رائع يا بطل! ربطتَ الأشياء بشكل صحيح 🎉"
      : wrongMatchId !== null
        ? "اقتربت! ابحث عن نفس العدد 👏"
        : "اسحب كل بطاقة إلى مكانها الصحيح 🍊";
  ...
  return (
    <div style={{
      ...
      {(feedbackState === "complete" || wrongMatchId !== null) && (
        <div style={{
          position: "fixed",
          left: 18,
          right: 18,
          ...
          color: C.navyDeep,
          border: `3px solid ${feedbackState === "complete" ? C.green : C.gold}`,
          borderRadius: 22,
          padding: "12px 16px",
          textAlign: "center",
          fontSize: 18,
