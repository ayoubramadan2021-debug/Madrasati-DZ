...
import UnifiedArithmeticDisplayV2 from "../components/UnifiedArithmeticDisplayV2";
...
export type DirectOperationQuestion = {
  question: string;
...
export type DirectOperationExerciseV2Props = {
  items: DirectOperationQuestion[];
  audio_base: string;
...
function playFeedback(
  ...
  try {
    ...
    // يبقى التمرين فعالًا دون المؤثر الصوتي.
  }
}

export default function DirectOperationExerciseV2({
  items,
  audio_base,
  background_image,
  onComplete,
}: DirectOperationExerciseV2Props) {
  ...
  return (
    <UnifiedExerciseScreenV2
      index={itemIndex}
      total={items.length}
      missionTitle="أُكَوِّنُ الْعَدَدَ"
      ...
      activityLabel="عملية حسابية مباشرة"
      answersLabel="اختيارات العدد الناقص"
    />
  );
}
