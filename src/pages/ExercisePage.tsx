...
function ExercisePage({ theme, setThemeName }: ExercisePageProps) {
  const { user } = useAuth();
  ...
  return (
    <Layout theme={theme} setThemeName={setThemeName} toast={favoriteMessage}>
      ...
      {isLoading ? (
        <LoadingState theme={theme} message="جاري تحميل التمرين..." />
      ) : isError ? (
        <ErrorState
          theme={theme}
          title="حدث خطأ أثناء تحميل التمرين"
        ...
        <EmptyState
          ...
          message="لم يتم العثور على هذا التمرين"
        />
      ) : (
        <>
