...
export default function AdminPage() {
  ...
  const [exTitleFr, setExTitleFr] = useState("");
  const [exInstrAr, setExInstrAr] = useState("");
  ...
  async function addExercise() {
    if (!exProblems.trim() || !exLessonId) {
      setExMsg("❌ اختر الدرس واكتب المسائل");
      setTimeout(() => setExMsg(""), 3000); return;
    }
    // تحويل "4+5, 8-3, 2*3, 6/2" إلى items
    const items = [];
    for (const raw of exProblems.split(",")) {
      const s = raw.trim();
      ...
      else if (op === "/") answer = b !== 0 ? a / b : 0;
      items.push({ a, b, op, answer });
    }
    if (items.length === 0) {
      setExMsg("❌ صيغة المسائل غير صحيحة (مثال: 4+5, 8-3)");
      setTimeout(() => setExMsg(""), 3000); return;
    }
    setLoading(true);
    ...
    const { error } = await supabase.from("exercises").insert([{
      type: "arithmetic",
      title: exTitleAr || "تمرين",
      subject: lesson?.subject || "math",
      grade: lesson?.grade || 1,
      lesson_number: lesson?.lesson_number ?? null,
      is_published: true,
      data: {
        title: { ar: exTitleAr, fr: exTitleFr },
        instruction: { ar: exInstrAr, fr: exInstrFr },
        items,
      },
    }]);
    if (error) { setExMsg("❌ " + error.message); }
    else {
      setExMsg("✅ تم إضافة " + items.length + " مسألة!");
      setExTitleAr(""); setExTitleFr(""); setExInstrAr(""); setExInstrFr(""); setExProblems("");
    }
    setLoading(false);
            ...
            {tab === 1 && (
              <>
                <div className="ad-card">
                  ...
                  <div className="ad-row2">
                    <input className="ad-input" value={exTitleAr} onChange={e => setExTitleAr(e.target.value)} placeholder="العنوان (عربي)" />
                    <input className="ad-input" value={exTitleFr} onChange={e => setExTitleFr(e.target.value)} placeholder="Titre (français)" style={{ direction: "ltr" }} />
                  </div>
                  <div className="ad-row2">
                    <input className="ad-input" value={exInstrAr} onChange={e => setExInstrAr(e.target.value)} placeholder="التعليمة (عربي)" />
                    <input className="ad-input" value={exInstrFr} onChange={e => setExInstrFr(e.target.value)} placeholder="Consigne (français)" style={{ direction: "ltr" }} />
                  </div>
                  <label className="ad-label">المسائل (افصل بفاصلة: 4+5, 8-3, 2*3, 6/2)</label>
                  <input className="ad-input" value={exProblems} onChange={e => setExProblems(e.target.value)} placeholder="4+5, 8-3, 2*3, 6/2" style={{ direction: "ltr", marginBottom: 10 }} />
