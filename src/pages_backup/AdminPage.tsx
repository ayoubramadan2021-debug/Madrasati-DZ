...
".ad-root{min-height:100dvh;background:#07101f;font-family:'Tajawal',sans-serif;direction:rtl;overflow-x:hidden;position:relative;padding-bottom:100px}",
...
".ad-back{position:absolute;top:20px;right:16px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);color:rgba(255,255,255,.8);border-radius:12px;padding:8px 14px;font-size:13px;font-weight:700;cursor:pointer;font-family:'Tajawal',sans-serif}",
...
".ad-logo-bg{position:absolute;inset:0;border-radius:22px;background:linear-gradient(145deg,#1a3d73,#0c1e3a);border:1px solid rgba(232,160,32,.4);box-shadow:0 8px 32px rgba(0,0,0,.5)}",
".ad-logo-em{position:relative;font-size:38px;filter:drop-shadow(0 0 14px rgba(232,160,32,.6))}",
".ad-title{font-size:23px;font-weight:900;line-height:1;margin-bottom:6px;background:linear-gradient(135deg,#fff 25%,#E8A020 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}",
".ad-sub{font-size:13px;color:rgba(255,255,255,.4)}",
".ad-body{padding:0 16px}",
".ad-stats{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:16px}",
".ad-stat{background:linear-gradient(135deg,rgba(15,25,45,.96),rgba(10,16,30,.98));border:1px solid rgba(255,255,255,.08);border-radius:14px;padding:14px 8px;text-align:center;box-shadow:0 4px 16px rgba(0,0,0,.3)}",
".ad-stat-em{font-size:22px;margin-bottom:4px}",
".ad-stat-n{font-size:22px;font-weight:900;line-height:1}",
".ad-stat-l{font-size:11px;color:rgba(255,255,255,.5);margin-top:4px}",
".ad-tabs{display:grid;grid-template-columns:repeat(3,1fr);background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:14px;padding:4px;margin-bottom:16px;gap:4px}",
".ad-tab{padding:11px 4px;border:none;border-radius:10px;background:transparent;color:rgba(255,255,255,.5);font-family:'Tajawal',sans-serif;font-size:13px;font-weight:700;cursor:pointer;transition:all .2s}",
".ad-tab.active{background:linear-gradient(135deg,#E8A020,#c97d0a);color:#000;box-shadow:0 4px 12px rgba(232,160,32,.3)}",
".ad-card{background:linear-gradient(135deg,rgba(15,25,45,.97),rgba(10,16,30,.99));border:1px solid rgba(255,255,255,.09);border-radius:18px;padding:20px;margin-bottom:14px;box-shadow:0 8px 28px rgba(0,0,0,.4)}",
".ad-card-t{font-weight:800;color:#E8A020;font-size:16px;margin-bottom:14px}",
...
".ad-input{width:100%;padding:11px 14px;border-radius:10px;border:1.5px solid rgba(255,255,255,.1);background:rgba(255,255,255,.04);color:#fff;font-family:'Tajawal',sans-serif;font-size:14px;outline:none;direction:rtl;transition:border-color .2s}",
".ad-input:focus{border-color:rgba(232,160,32,.5)}",
".ad-input::placeholder{color:rgba(255,255,255,.3)}",
...
".ad-sec-bar{width:4px;height:20px;border-radius:4px;background:linear-gradient(180deg,#E8A020,#c97d0a)}",
".ad-sec-t{font-size:16px;font-weight:800;color:#fff}",
".ad-list{display:flex;flex-direction:column;gap:10px}",
".ad-item{background:linear-gradient(135deg,rgba(15,25,45,.96),rgba(10,16,30,.98));border:1px solid rgba(255,255,255,.08);border-radius:14px;padding:14px 16px;display:flex;align-items:center;gap:12px;box-shadow:0 4px 14px rgba(0,0,0,.3)}",
".ad-item-main{flex:1;min-width:0}",
".ad-item-t{font-weight:700;color:#fff;font-size:14px}",
".ad-item-s{color:rgba(255,255,255,.5);font-size:12px;margin-top:2px}",
...
".ad-rank{width:34px;height:34px;border-radius:50%;background:linear-gradient(135deg,#1a3d73,#0c1e3a);border:1px solid rgba(232,160,32,.3);color:#E8A020;display:grid;place-items:center;font-weight:800;flex-shrink:0;font-size:14px}",
".ad-pts{font-weight:900;color:#E8A020;font-size:15px;flex-shrink:0}",
".ad-empty{text-align:center;padding:30px;color:rgba(255,255,255,.4);font-size:14px}",
".ad-statgrid{display:grid;grid-template-columns:1fr 1fr;gap:14px}",
".ad-statbox{background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:14px;padding:18px;text-align:center}",
".ad-statbox-n{font-size:26px;font-weight:900;line-height:1}",
".ad-statbox-l{color:rgba(255,255,255,.5);font-size:12px;margin-top:6px}",
].join("\n");
...
export default function AdminPage() {
  ...
  const statCards = [
    ...
    { label: "النشاط", value: stats.progress, icon: "📊", color: "#E8A020" },
  ];

  if (authorized === null) {
    return (<div style={{ minHeight: "100dvh", background: "#07101f", color: "#E8A020", display: "grid", placeItems: "center", fontFamily: "Tajawal, sans-serif", fontSize: 16 }}>⏳ جاري التحقق...</div>);
  }
  if (authorized === false) {
    return (<div style={{ minHeight: "100dvh", background: "#07101f", color: "#fff", display: "grid", placeItems: "center", textAlign: "center", padding: 30, fontFamily: "Tajawal, sans-serif", direction: "rtl" }}><div><div style={{ fontSize: 60, marginBottom: 16 }}>🔒</div><div style={{ fontSize: 18, fontWeight: 800, marginBottom: 8 }}>غير مصرّح لك</div><div style={{ color: "rgba(255,255,255,.5)", fontSize: 14 }}>هذه الصفحة مخصّصة للإدارة فقط</div><button onClick={() => navigate("/")} style={{ marginTop: 20, padding: "12px 24px", border: "none", borderRadius: 12, background: "linear-gradient(135deg,#E8A020,#c97d0a)", color: "#000", fontFamily: "Tajawal, sans-serif", fontWeight: 800, fontSize: 14 }}>العودة للرئيسية</button></div></div>);
  }
              ...
              <>
                <div className="ad-card">
                  ...
                  <button className="ad-add" onClick={addLesson} disabled={!title.trim() || loading}
                    style={{ background: title.trim() ? "linear-gradient(135deg,#E8A020,#c97d0a)" : "rgba(255,255,255,.1)", color: title.trim() ? "#000" : "rgba(255,255,255,.4)", cursor: title.trim() ? "pointer" : "default" }}>
              ...
              <div className="ad-card">
                ...
                <div className="ad-statgrid">
                  {[
                    ...
                    { label: "إجمالي النشاط", value: stats.progress, color: "#E8A020" },
