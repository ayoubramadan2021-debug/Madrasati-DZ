...
".sc-root{min-height:100dvh;background:#07101f;font-family:'Tajawal',sans-serif;direction:rtl;overflow-x:hidden;position:relative;padding-bottom:100px}",
...
".sc-back{position:absolute;top:20px;right:16px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);color:rgba(255,255,255,.8);border-radius:12px;padding:8px 14px;font-size:13px;font-weight:700;cursor:pointer;font-family:'Tajawal',sans-serif;transition:background .2s}",
".sc-back:active{background:rgba(255,255,255,.12)}",
...
".sc-logo-bg{position:absolute;inset:0;border-radius:24px;background:linear-gradient(145deg,#1a3d73,#0c1e3a);border:1px solid rgba(232,160,32,.4);box-shadow:0 8px 32px rgba(0,0,0,.5),inset 0 1px 0 rgba(255,255,255,.08)}",
...
".sc-title{font-size:26px;font-weight:900;line-height:1;margin-bottom:6px;background:linear-gradient(135deg,#fff 25%,#E8A020 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;opacity:0;transform:translateY(14px);transition:opacity .6s ease .2s,transform .6s ease .2s}",
".sc-title.in{opacity:1;transform:translateY(0)}",
".sc-sub{font-size:13px;color:rgba(255,255,255,.4);opacity:0;transition:opacity .6s ease .35s}",
".sc-sub.in{opacity:1}",
".sc-body{padding:0 16px}",
".sc-list{display:flex;flex-direction:column;gap:12px}",
".sc-item{position:relative;background:linear-gradient(135deg,rgba(15,25,45,.96),rgba(10,16,30,.98));border:1px solid rgba(255,255,255,.08);border-radius:16px;padding:15px 16px;display:flex;align-items:center;gap:14px;cursor:pointer;overflow:hidden;animation:sc-slide .5s ease backwards;transition:transform .2s}",
...
".sc-item-t{font-weight:700;color:#fff;font-size:15px}",
".sc-item-d{color:rgba(255,255,255,.4);font-size:12px;margin-top:3px}",
".sc-item-ar{width:32px;height:32px;border-radius:50%;color:#fff;display:grid;place-items:center;font-weight:700;flex-shrink:0;font-size:14px}",
".sc-empty{text-align:center;padding:50px 20px;color:rgba(255,255,255,.4)}",
              ...
              <div className="sc-list">
                ...
                {exercises.map((ex: any, i: number) => (
                  <div key={ex.id} className="sc-item" style={{ animationDelay: (0.3 + i * 0.07) + "s", boxShadow: "0 4px 18px rgba(0,0,0,.3)" }}
                    ...
                    <div className="sc-item-strip" style={{ background: "linear-gradient(180deg,#E8A020,#c97d0a)" }} />
                    ...
                    <div className="sc-item-ar" style={{ background: "#E8A020" }}>←</div>
                  </div>
                ))}
              </div>
            )}
