...
export default function ConservationV2({ items, audio_base, onComplete }: ConservationV2Props) {
  ...
  const progressEmoji = "🧠";
  const missionText = `مهمة التفكير ${itemIdx + 1}`;

  const coachText =
    feedbackState === "correct"
      ? "رائع يا بطل! فهمتَ أن الكمية لا تتغير 🎉"
      : feedbackState === "wrong"
        ? "اقتربت! غيّرنا الشكل فقط، فكّر من جديد 👏"
        : rearranged
          ? "فكّر جيدًا: هل تغيّر العدد أم تغيّر الشكل فقط؟ 🧠"
          : "اضغط جمّعها ثم لاحظ الكمية 🧲";
  ...
  return (
    <div style={{
              ...
              return (
                <button key={opt} onClick={() => handleSelect(opt)} disabled={locked}
                  style={{
                    ...
                    color: C.navy, transition: "all .25s ease", boxShadow: showAsCorrect ? "0 10px 26px rgba(31,164,99,.35)" : isSelected ? "0 6px 16px rgba(0,0,0,.22)" : "0 4px 12px rgba(0,0,0,.12)", transform: showAsCorrect ? "translateY(-4px) scale(1.06)" : showAsWrong ? "scale(0.96)" : "scale(1)",
                  }}>{opt}</button>
              );
            })}
          </div>
      ...
      <style>{`@keyframes popIn { 0%{opacity:0;transform:translateY(-20px) scale(0.4);} 60%{opacity:1;transform:translateY(2px) scale(1.1);} 100%{opacity:1;transform:translateY(0) scale(1);} } @keyframes feedbackPop { 0%{opacity:0;transform:scale(.7);} 60%{opacity:1;transform:scale(1.08);} 100%{opacity:1;transform:scale(1);} }`}</style>
    </div>
  );
}
