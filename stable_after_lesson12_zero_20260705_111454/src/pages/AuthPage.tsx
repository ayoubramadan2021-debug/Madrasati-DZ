...
".au-pw-wrap{position:relative;margin-bottom:20px}",
".au-pw-wrap .au-field{margin-bottom:0;padding-left:44px;width:100%}",
".au-pw-eye{position:absolute;left:12px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:18px;padding:0;line-height:1}",
].join("\n");
  ...
  async function handleSubmit() {
    ...
    try {
      if (isLogin) {
        ...
        setSuccess("تم انشاء الحساب! تحقق من بريدك الالكتروني.");
      }
    } catch (e) {
    ...
    <>
      ...
      <div className="au-root">
        ...
        <div className={cx("au-card", mounted && "in")}>
          ...
          <label className="au-label">البريد الالكتروني</label>
          ...
          <button className="au-submit" onClick={handleSubmit} disabled={loading}>
            {loading ? "جاري المعالجة..." : isLogin ? "تسجيل الدخول" : "انشاء الحساب"}
          </button>
