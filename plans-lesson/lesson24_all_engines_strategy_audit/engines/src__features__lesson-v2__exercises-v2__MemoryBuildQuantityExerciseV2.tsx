...
export default function MemoryBuildQuantityExerciseV2({
  ...
  const progressEmoji = "🧩";
  const missionText = `مهمة البناء ${itemIdx + 1}`;

  const coachText =
    feedback === "correct"
      ? "رائع يا بطل! بنيتَ نفس الكمية 🎉"
      : feedback === "wrong"
        ? "اقتربت! تذكّر الكمية التي رأيتها 👏"
        : phase === "show"
          ? "انظر جيدًا واحفظ الكمية بعينيك 🧠"
          : "ابنِ نفس الكمية التي شاهدتها 🧩";
