...
TARGET="$PROJECT/src/features/lesson-v2/exercises-v2/PremiumExerciseEngineV2.tsx"
BACKUP="$PROJECT/backups/premium_engine_$(date '+%Y%m%d_%H%M%S')"
BUILD_LOG="/sdcard/Download/premium_engine_v2_build.log"

mkdir -p "$(dirname "$TARGET")" "$BACKUP" "/sdcard/Download"

if [ -f "$TARGET" ]; then
  cp "$TARGET" "$BACKUP/PremiumExerciseEngineV2.tsx"
  echo "✅ تم حفظ نسخة احتياطية من المحرك السابق."
fi
...
if npm run build > "$BUILD_LOG" 2>&1; then
  echo "✅ تم إنشاء PremiumExerciseEngineV2."
  echo "✅ المحرك يدعم Full Screen."
  echo "✅ الرأس الموحد مربوط."
  echo "✅ الكاريوكي وتوقيت الكلمات مدعومان."
  echo "✅ نافذة أحسنت/حاول مرة أخرى موحدة."
  echo "✅ الإجابة الصحيحة تنتقل تلقائيًا."
  echo "✅ الإجابة الخاطئة تسمح بإعادة المحاولة."
  echo "✅ npm build ناجح."
  echo "الملف: $TARGET"
else
  echo "❌ فشل البناء."
