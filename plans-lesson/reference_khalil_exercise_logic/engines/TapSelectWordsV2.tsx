...
export default function TapSelectWordsV2({
  ...
  const progressEmoji = "⭐";
  const missionText = `مهمة الاختيار ${itemIdx + 1}`;

  const coachText =
    feedbackState === "correct"
      ? "رائع يا بطل! اخترتَ الكلمة الصحيحة 🎉"
      : feedbackState === "wrong"
        ? "اقتربت! اقرأ الكلمة مرة أخرى 👏"
        : "استمع جيدًا واختر الكلمة المناسبة ⭐";
          ...
          return (
            <button
              ...
              style={{
                ...
                boxShadow: showAsCorrect ? "0 10px 26px rgba(31,164,99,.35)" : isSelected ? "0 6px 16px rgba(0,0,0,.25)" : "0 4px 12px rgba(0,0,0,.1)",
                transform: showAsCorrect ? "translateY(-4px) scale(1.08)" : showAsWrong ? "scale(0.95)" : "scale(1)",
