...
export default function RankOrderExerciseV2({
  ...
  const handleAnswer = (value: string) => {
    ...
    if (ok) {
      ...
      setScore(newScore);
      setTimeout(() => nextItem(newScore), 1800);
    } else {
      setFeedback("wrong");
      playFeedback(false);
      setTimeout(() => {
  ...
  const coachText =
    feedback === "correct"
      ? "رائع يا بطل! اخترتَ الترتيب الصحيح 🎉"
      : feedback === "wrong"
        ? "اقتربت! انظر إلى الرتبة جيدًا 👏"
        : "ركّز على الترتيب ثم اختر الإجابة 🏆";
    ...
    <main dir="rtl" style={styles.page}>
      <section style={styles.screen}>
        ...
        <header style={styles.header}>
          <div style={styles.counter}><span dir="ltr">{itemIdx + 1} / {items.length}</span></div>
        ...
        {feedback !== "idle" && (
          <div style={{
            ...
            color: "#0F2447",
            border: `3px solid ${feedback === "correct" ? "#1FA463" : "#E8A020"}`,
            borderRadius: 22,
            padding: "12px 16px",
            textAlign: "center",
            fontSize: 18,
